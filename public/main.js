...
const appSettings = {
    ...
    contextHoldDelayMs: 500, // Long-press delay for touch context menu
    // Coords & Bookmarks visibility
    enableCoords: false,
};
...
function loadSettings() {
    ...
    const holdDelayEl = document.getElementById('context-hold-delay-input');
    if (holdDelayEl) holdDelayEl.value = appSettings.contextHoldDelayMs;

    // Apply coords/bookmarks visibility
    const toggleCoordsEl = document.getElementById('toggle-coords');
    if (toggleCoordsEl) {
        toggleCoordsEl.checked = !!appSettings.enableCoords;
    }
    applyCoordsEnabledState();
}

function applyCoordsEnabledState() {
    const enabled = !!appSettings.enableCoords;
    const zoomIndicator = document.getElementById('zoom-indicator');
    const bookmarksBlock = document.getElementById('bookmarks-block');
    const bookmarksPanel = document.getElementById('bookmarks-panel');
    const bookmarksToggleBtn = document.getElementById('toggle-bookmarks-panel');
    if (zoomIndicator) {
        zoomIndicator.style.display = enabled ? 'block' : 'none';
    }
    if (bookmarksBlock) {
        bookmarksBlock.style.display = enabled ? '' : 'none';
    }
    if (!enabled) {
        // Ensure panel is hidden when disabled
...
function setupEventListeners() {
    ...
    renderer.domElement.addEventListener('touchstart', (event) => {
        if (event.touches.length === 1) {
            ...
            }, Math.max(0, parseInt(appSettings.contextHoldDelayMs || 500))); // configurable long-press delay
        }
    }, { passive: false });
    ...
    const holdDelayEl2 = document.getElementById('context-hold-delay-input');
    if (holdDelayEl2) {
        holdDelayEl2.addEventListener('input', (event) => {
            const v = parseInt(event.target.value, 10);
            // Clamp to sensible range 0..5000ms
            appSettings.contextHoldDelayMs = isNaN(v) ? 500 : Math.max(0, Math.min(5000, v));
            event.target.value = appSettings.contextHoldDelayMs;
            saveSettings();
        });
    }
    ...
    const toggleCoordsEl = document.getElementById('toggle-coords');
    if (toggleCoordsEl) {
        toggleCoordsEl.addEventListener('change', (event) => {
            appSettings.enableCoords = event.target.checked;
            applyCoordsEnabledState();
            saveSettings();
        });
    }
